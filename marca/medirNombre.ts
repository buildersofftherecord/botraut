...
/**
 * Parsea Archivo-900-125.ttf con `@shuding/opentype.js`: la misma librería
 ...
 */
function cargarFontArchivo(): Promise<Font> {
  if (!fontePromise) {
    fontePromise = readFile(join(aca, "fuentes/Archivo-900-125.ttf")).then((buf) => {
...
/**
 * Ancho real, en píxeles, que ocupa `texto` en Archivo 900 a `fontSize`, en
...
export async function anchoTexto(texto: string, fontSize: number): Promise<number> {
  const font = await cargarFontArchivo();
